...
public class ActualFinalClassCheck extends AbstractCheck {
    ...
    public void visitToken(DetailAST ast) {
        ...
        case TokenTypes.CLASS_DEF:
            ...
            this.nonFinalClasses.put(
                    ...
                    new Frame(this.fileName, ast.getLineNo(), CommonUtil.lengthExpandedTabs(
    ...
    public void finishProcessing() {
        for (String clss : this.nonFinalClasses.keySet()) {
            if (!this.inheritedClasses.contains(clss)) {
                ...
                final String filePath = frame.getFileName();
                final MessageDispatcher dispatcher = getMessageDispatcher();

                dispatcher.fireFileStarted(filePath);
                logExternal(filePath, frame.getLineNo(), frame.getColumnNo(), MSG_KEY);
                fireErrors(filePath);
     ...
     * 
     * @param ast
     *            - DetailAST node is pointing to import definition or to the "new" literal node
     *            ("IMPORT" or "LITERAL_NEW" node types).
     * @return Import text without "import" word and semicolons for given "IMPORT" node or
     *         instanstiated class Name&Path for given "LITERAL_NEW" node.
     */
    public static String getText(final DetailAST ast) {
        String result = null;
